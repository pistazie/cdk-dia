...
/**
 * Copy of the AWS CDK `makeUniqueId` helper (packages/aws-cdk-lib/core/lib/private/uniqueid.ts).
 * Reproduced here to avoid depending on CDK private paths, while keeping behaviour identical.
 * The surrounding helper functions (`md5hash`, `unresolved`, etc.) are minimal shims that
 * preserve the original semantics for cdk-dia usage. */

// Minimal replacements for CDK internals --------------------------------------
class UnscopedValidationError extends Error {}

function unresolved(x: string): boolean {
  // Simplistic check for unresolved CloudFormation tokens (e.g. "${Token[...]}").
  return /\${Token\[.*?]}/.test(x);
}
